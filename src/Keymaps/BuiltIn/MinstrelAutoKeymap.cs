namespace DavidRice.BlishHud.MidiControl.Keymaps.BuiltIn
{
    ...
    {
        public static Keymap Instance { get; } = new Keymap("minstrel-auto", "The Minstrel (Auto)")
        {
            AutoOctaveSwap = true,
            OctaveDownKey = "9",
            OctaveUpKey = "0",
            Notes =
            {
                ...
                { "F#4", new NoteDefinition(forceInternalOctave: 0) },
                { "G#4", new NoteDefinition(forceInternalOctave: 1) },
                { "A#4", new NoteDefinition(forceInternalOctave: 2) }
            }
        };
    }
}
